var c={userId:0,admin:!1},o=class extends HTMLElement{static style="";static register(e){let t=document.createElement("style");t.textContent=this.style,document.head.appendChild(t),customElements.define(e,this)}Query(e){return this.querySelector(e)}Listen(e,t,r){this.addEventListener(e,t,r)}};async function S(n,e,t={}){let r=await fetch("/api",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n,targetId:e,payload:t})});if(!r.ok)throw r.status===401&&window.dispatchEvent(new CustomEvent("nerd:unauthorized")),new Error(await r.text()||"Request failed");return await r.json()}async function h(n,e){let t=await fetch("/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n,payload:e})});if(!t.ok)throw new Error(await t.text()||"Request failed");return await t.json()}async function v(n){return await S(0,n)}var L=class{rootId;openList;displayRoot},x=class{trees},C=class{boards},a=class{workbench},m={workbench:{boards:[{trees:[{rootId:0,openList:new Set,displayRoot:1}]},{trees:[{rootId:0,openList:new Set,displayRoot:2}]}]}};var I=/^\s+|\s+$|(?<=\>)\s+(?=\<)/gm;function u(n){let e=document.createElement("template");return e.innerHTML=n.replace(I,""),e.content.firstElementChild}var l=class n{id;name;parent;children;elements;constructor(e,t,r=null){this.id=e,this.name=t,this.parent=r,this.children=[],this.elements=[]}addChild(e,t){let r=new n(e,t,this);return this.children.push(r),r}collectToDepth(e,t){if(t.add(this.id),e>0)for(let r of this.children)r.collectToDepth(e-1,t)}render(e,t){let r=u(`<div class="nerd-entity">${this.name}</div>`);if(this.elements.push(r),e.appendChild(r),t.openList.has(this.id)){let i=u('<div class="nerd-children"></div>');r.appendChild(i);for(let s of this.children)s.render(i,t)}return r}};var p=class extends o{static style=`
		nerd-action {
			display: inline;
			background: none;
			border: none;
	...
	`};p.register("nerd-action");var d,g=class extends o{static style=`
		nerd-vertigo {
			display: block;
		}
