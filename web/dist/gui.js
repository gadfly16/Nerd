var x=/^\s+|\s+$|(?<=\>)\s+(?=\<)/gm;function u(n){let e=document.createElement("template");return e.innerHTML=n.replace(x,""),e.content.firstElementChild}var g={userId:0,admin:!1},o=class extends HTMLElement{static style="";static register(e){let t=document.createElement("style");t.textContent=this.style,document.head.appendChild(t),customElements.define(e,this)}Query(e){return this.querySelector(e)}Listen(e,t,r){this.addEventListener(e,t,r)}};async function v(n,e,t={}){let r=await fetch("/api",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n,targetId:e,payload:t})});if(!r.ok)throw r.status===401&&window.dispatchEvent(new CustomEvent("nerd:unauthorized")),new Error(await r.text()||"Request failed");return await r.json()}async function m(n,e){let t=await fetch("/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:n,payload:e})});if(!t.ok)throw new Error(await t.text()||"Request failed");return await t.json()}var d=class{rootId;stopList},a=class{listTrees},h=class{boards},l=class{workbench};var f=class extends o{static style=`
	...
	`};f.register("nerd-action");var c,p=class n{id;name;parent;children;elements;constructor(e,t,r=null){this.id=e,this.name=t,this.parent=r,this.children=[],this.elements=[]}addChild(e,t){let r=new n(e,t,this);return this.children.push(r),r}render(e,t){let r=u(`<div class="nerd-entity">${this.name}</div>`);if(this.elements.push(r),e.appendChild(r),!t.stopList.has(this.id)){let i=u('<div class="nerd-children"></div>');r.appendChild(i);for(let s of this.children)s.render(i,t)}return r}},b=class extends o{static style=`
		nerd-list-tree {
			display: block;
		}
	...
	`;logoutButton;connectedCallback(){this.innerHTML=n.html,this.logoutButton=this.Query(".logout"),this.logoutButton.addEventListener("click",()=>this.logout())}async logout(){try{await m(6,{}),c.SwitchToAuth()}catch(e){console.error("Logout failed:",e)}}},y=class n extends o{static style=`
		nerd-footer {
			display: block;
			background: #333;
			color: white;
	...
	`;static html=`
		Footer
	`;connectedCallback(){this.innerHTML=n.html}},L=class n extends o{static style=`
		nerd-workbench {
			display: grid;
	...
	`;boardElements=[];connectedCallback(){this.innerHTML=n.html;let e=this.Query("nerd-board.left"),t=this.Query("nerd-board.right");this.boardElements=[e,t]}Init(e){let t=new h;t.boards=[];let r=0;for(let i of this.boardElements){let s=i.Init(e?.boards[r]??null);t.boards.push(s),r++}return t}RenderBoards(){for(let e of this.boardElements)e.Render()}},k=class n extends o{static style=`
		nerd-auth {
			display: flex;
	...
	`;regmode=!1;login=void 0;register=void 0;error=void 0;loginToggle=void 0;registerToggle=void 0;connectedCallback(){this.innerHTML=n.html,this.login=this.Query(".login"),this.register=this.Query(".register"),this.error=this.Query(".error"),this.loginToggle=this.login.querySelector(".toggle"),this.registerToggle=this.register.querySelector(".toggle"),this.attachEventListeners()}attachEventListeners(){this.login.addEventListener("submit",e=>this.handleSubmit(e,!1)),this.register.addEventListener("submit",e=>this.handleSubmit(e,!0)),this.loginToggle.addEventListener("click",()=>this.toggleMode()),this.registerToggle.addEventListener("click",()=>this.toggleMode())}toggleMode(){this.regmode=!this.regmode,this.login.classList.toggle("hidden"),this.register.classList.toggle("hidden")}async handleSubmit(e,t){e.preventDefault();let r=new FormData(e.target),i=Object.fromEntries(r);try{let s=await m(t?5:4,i);c.SwitchToWorkbench(s.userid)}catch(s){this.showError(s instanceof Error?s.message:"Network error. Please try again.")}}showError(e){this.error.textContent=e}},E=class n extends o{static style=`
		@font-face {
			font-family: 'Inter';
